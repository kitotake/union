...
RegisterCommand("myjob", function()
    ...
    Notifications.send(string.format("Job : %s | Grade : %s", job, grade), "info")
end, false)
...
RegisterNetEvent("union:job:employees:received", function(job, employees)
    ...
    for _, e in ipairs(employees) do
        print(string.format("  ^3%s %s^7 | Grade : %s | UID : %s",
            e.firstname or "?", e.lastname or "?", e.job_grade or 0, e.unique_id or "?"))
    end
...
RegisterNetEvent("union:job:updated", function(job, grade)
    ...
    Notifications.send(string.format("Votre job a été mis à jour : %s (grade %s)", job, grade), "success")
end)
