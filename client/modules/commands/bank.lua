...
RegisterNetEvent("union:bank:balanceReceived", function(balance)
    Notifications.send(string.format("Solde : $%s", balance), "info")
end)

RegisterNetEvent("union:bank:depositResult", function(success, amount, balance)
    if success then
        Notifications.send(string.format("Dépôt de $%s effectué. Nouveau solde : $%s", amount, balance), "success")
    else
...
RegisterNetEvent("union:bank:withdrawResult", function(success, amount, balance)
    if success then
        Notifications.send(string.format("Retrait de $%s effectué. Nouveau solde : $%s", amount, balance), "success")
    else
...
RegisterNetEvent("union:bank:transferResult", function(success, amount)
    if success then
        Notifications.send(string.format("Transfert de $%s effectué.", amount), "success")
    else
...
RegisterNetEvent("union:bank:transactionsReceived", function(transactions)
    ...
    for _, t in ipairs(transactions) do
        ...
        print(string.format("  %s$%s^7 — %s — %s", sign, t.amount, t.type, t.description or "N/A"))
    end
