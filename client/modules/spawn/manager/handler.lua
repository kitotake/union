...
RegisterNetEvent("union:spawn:apply", function(characterData)
    ...
    Citizen.CreateThread(function()
        ...
        -- 4. APPARENCE
        -- FIX PERF (chargement perso lent) : l'attente de kt_character était
        -- jusqu'à 20 x 250ms = 5000ms. Si la ressource kt_character démarre
        -- après "union" ou tarde à s'initialiser, CHAQUE spawn payait ce
        -- délai en entier avant de tomber sur le fallback. On réduit la
        -- fenêtre d'attente à 1.5s max et on log explicitement le temps
        -- réel attendu pour pouvoir confirmer en jeu si c'est bien la cause
        -- du ralentissement perçu (regarder la console pour
        -- "[BRIDGE] kt_character pas prêt après ...").
        local waitStart = GetGameTimer()
        local waited     = 0
        while not Bridge.Character:isAvailable() and waited < 15 do
            Wait(100)
            waited = waited + 1
        end
        local waitedMs = GetGameTimer() - waitStart
        if not Bridge.Character:isAvailable() then
            Logger:warn(("kt_character pas prêt après %dms — fallback modèle de base"):format(waitedMs))
        elseif waitedMs > 0 then
