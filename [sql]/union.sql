...
CREATE TABLE IF NOT EXISTS `kt_inventory` (
    ...
    CONSTRAINT `fk_inventory_char` FOREIGN KEY (`unique_id`) REFERENCES `characters` (`unique_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `owned_vehicles` (
    ...
    INDEX `idx_unique_id` (`unique_id`),
    CONSTRAINT `fk_vehicle_char` FOREIGN KEY (`unique_id`) REFERENCES `characters` (`unique_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `job_grades` (
    ...
    CONSTRAINT `fk_jobgrade_job` FOREIGN KEY (`job_name`) REFERENCES `jobs` (`name`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `user_licenses` (
    ...
    INDEX `idx_identifier` (`identifier`),
    CONSTRAINT `fk_userlicense_char` FOREIGN KEY (`unique_id`) REFERENCES `characters` (`unique_id`) ON DELETE CASCADE,
    CONSTRAINT `fk_userlicense_type` FOREIGN KEY (`type`) REFERENCES `licenses` (`type`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `bank_transactions` (
    ...
    CONSTRAINT `fk_transaction_target` FOREIGN KEY (`target_account_id`) REFERENCES `bank_accounts` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `bank_limits` (
    ...
    CONSTRAINT `fk_limit_account` FOREIGN KEY (`account_id`) REFERENCES `bank_accounts` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `action_logs` (
    ...
    CONSTRAINT `fk_actionlog_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_actionlog_char` FOREIGN KEY (`character_id`) REFERENCES `characters` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
('unemployed','Chômeur',0),('police','Police',1),('ambulance','Ambulance',1),
('mechanic','Mécanicien',0),('security','Sécurité',0),('taxi','Taxi',0);

INSERT IGNORE INTO `job_grades` (`job_name`,`grade`,`name`,`label`,`salary`) VALUES
('unemployed',0,'unemployed','Chômeur',50),
('police',0,'recruit','Recrue',200),('police',1,'officer','Officier',250),
('police',2,'sergeant','Sergent',300),('police',3,'lieutenant','Lieutenant',350),
('police',4,'captain','Capitaine',400),('police',5,'chief','Chef',450),
('ambulance',0,'nurse','Infirmier',200),('ambulance',1,'doctor','Médecin',300),
('ambulance',2,'surgeon','Chirurgien',400),('ambulance',3,'chief','Chef',450),
('mechanic',0,'apprentice','Apprenti',100),('mechanic',1,'novice','Novice',150),
('mechanic',2,'experienced','Expérimenté',200),('mechanic',3,'expert','Expert',250),
('mechanic',4,'boss','Patron',300),
('security',0,'guard','Garde',150),('security',1,'supervisor','Superviseur',200),('security',2,'manager','Gérant',250),
('taxi',0,'driver','Chauffeur',100),('taxi',1,'owner','Propriétaire',200);

INSERT IGNORE INTO `licenses` (`type`,`label`) VALUES
('dmv','Driving Permit'),('drive','Drivers License'),('drive_bike','Motorcycle License'),
('drive_truck','Truck License'),('drive_taxi','Taxi License'),
('weapon','Weapon License'),('hunt','Hunting License'),('fish','Fishing License');
