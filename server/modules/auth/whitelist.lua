...
RegisterCommand("wl:add", function(source, args)
    ...
    if not license then print("^1Usage: /wl:add <license>^7"); return end
    if src ~= 0 then
    ...
    Whitelist.add(license, addedBy, function(success)
        local msg = success and ("Whitelist ajouté : " .. license) or ("Échec whitelist pour : " .. license)
        print((success and "^2" or "^1") .. "[WHITELIST] " .. msg .. "^7")
        if src ~= 0 then ServerUtils.notifyPlayer(src, msg, success and "success" or "error") end
    end)
end, true)
...
RegisterCommand("wl:remove", function(source, args)
    local src     = source
    local license = args[1]
    if not license then print("^1Usage: /wl:remove <license>^7"); return end
    if src ~= 0 then
        local admin = PlayerManager.get(src)
        if not admin or not admin:hasPermission("admin.all") then
            ServerUtils.notifyPlayer(src, "Permission refusée.", "error"); return
    ...
    Whitelist.remove(license, function(success)
        local msg = success and ("Whitelist retiré : " .. license) or ("Échec retrait pour : " .. license)
        print((success and "^2" or "^1") .. "[WHITELIST] " .. msg .. "^7")
        if src ~= 0 then ServerUtils.notifyPlayer(src, msg, success and "success" or "error") end
    end)
end, true)
