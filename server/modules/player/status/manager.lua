...
local function defaultStatus()
    return {
        ...
        _dirty = false, _uniqueId = nil, _pendingSend = false,
    }
end
...
function StatusManager.load(src, uniqueId, callback)
    if StatusManager._loading[src] then
        ...
        CreateThread(function()
            local waited = 0
            while StatusManager._loading[src] and waited < 30 do Wait(100); waited = waited + 1 end
            if callback then callback(StatusManager.cache[src]) end
        end)
        return
    end
    StatusManager._loading[src] = true
    exports.oxmysql:fetch("SELECT hunger, thirst, stress FROM player_status WHERE unique_id = ?", { uniqueId }, function(rows)
        ...
        if GetPlayerEndpoint(src) then
            TriggerClientEvent("union:status:init", src, { hunger = status.hunger, thirst = status.thirst, stress = status.stress })
        end
...
function StatusManager.save(src, status, identifier)
    if not status or not status._dirty then return end
    if not status._uniqueId or status._uniqueId == "" then
        StatusManager.logger:warn(("save: _uniqueId nil src=%d"):format(src)); return
    end
    local license = identifier
    if not license then
        local player = PlayerManager.get(src)
        if not player or not player.license then
            StatusManager.logger:warn(("save: license nil src=%d"):format(src)); return
        end
        license = player.license
    end
    if not license or license == "" then
        StatusManager.logger:warn(("save: license vide src=%d"):format(src)); return
    end
    exports.oxmysql:execute([[
        INSERT INTO player_status (unique_id, hunger, thirst, stress, last_update) VALUES (?, ?, ?, ?, NOW())
        ON DUPLICATE KEY UPDATE hunger = VALUES(hunger), thirst = VALUES(thirst), stress = VALUES(stress), last_update = NOW()
    ]], { status._uniqueId, clamp(status.hunger), clamp(status.thirst), clamp(status.stress) })
    status._dirty = false
end

function StatusManager.get(src)   return StatusManager.cache[src] end

function StatusManager.set(src, stat, value)
    if not ALLOWED_STATS[stat] then return end
    local s = StatusManager.cache[src]
    if not s then return end
    s[stat] = clamp(value); s._dirty = true; s._pendingSend = true
end
...
function StatusManager.flushPendingSends()
    for src, status in pairs(StatusManager.cache) do
        if status and status._pendingSend then
            if GetPlayerEndpoint(src) then
                TriggerClientEvent("union:status:updateAll", src, { hunger = status.hunger, thirst = status.thirst, stress = status.stress })
            end
...
AddEventHandler("union:player:dropping", function(src, player, reason)
    ...
    if status then StatusManager.save(src, status, license); StatusManager.cache[src] = nil end
end)
...
exports("AddStat", function(src, stat, value)
    ...
    local player = PlayerManager.get(src)
    StatusManager.save(src, s, player and player.license)
end)