...
function Persistence.savePlayer(player)
    ...
    elseif type(posData) == "string" then
        ...
        if ok and p and p.x then px, py, pz = p.x, p.y, p.z; heading = p.heading or char.heading or 0.0 end
...
function Persistence.saveAllPlayers()
    ...
    if saved > 0 then Persistence.logger:debug(("Backup save: %d joueur(s)"):format(saved)) end
end
