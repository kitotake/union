...
function Job.setPlayerJob(player, jobName, grade, callback)
    if not player or not jobName then if callback then callback(false) end; return end
    Database.execute("UPDATE characters SET job = ?, job_grade = ? WHERE unique_id = ?",
        { jobName, grade or 0, player.currentCharacter.unique_id }, function(result)
            if result then
...
function Job.getPlayerJob(player)
    if not player or not player.currentCharacter then return nil, 0 end
    return player.currentCharacter.job or "unemployed", player.currentCharacter.job_grade or 0
end

function Job.getJobs(callback)
    Database.fetch("SELECT * FROM jobs", {}, function(result) if callback then callback(result or {}) end end)
end

function Job.getJobGrades(jobName, callback)
    Database.fetch("SELECT * FROM job_grades WHERE job_name = ? ORDER BY grade ASC", { jobName },
        function(result) if callback then callback(result or {}) end end)
end
...
RegisterNetEvent("union:job:set", function(jobName, grade)
    ...
    if player and player:hasPermission("job.set") then
        Job.setPlayerJob(player, jobName, grade, function(success)
            if success then TriggerClientEvent("union:job:updated", src, jobName, grade) end
        end)
    else
        if player then player:notify("Permission refusée.", "error") end
    end
end)

RegisterNetEvent("union:job:list:request", function()
    local src = source
    Job.getJobs(function(jobs) TriggerClientEvent("union:job:list:received", src, jobs) end)
end)
...
RegisterNetEvent("union:job:employees:request", function(jobName)
    ...
    Database.fetch("SELECT firstname, lastname, job_grade, unique_id FROM characters WHERE job = ?",
        { jobName }, function(employees)
