-- /setgroup <id> <group> — changer le groupe d'un joueur (admin)
RegisterCommand("setgroup", function(source, args)
    ...
    if not targetId or not groupName then
        if src == 0 then
            print("Usage: setgroup <id> <group>")
        else
            ServerUtils.notifyPlayer(src, "Usage: /setgroup <id> <group>", "error")
        end
        return
    end
    ...
    if not PermissionGroups.defaults[groupName] then
        local valid = {}
        for k in pairs(PermissionGroups.defaults) do
