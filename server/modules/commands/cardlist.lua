...
RegisterCommand("cardlist", function(source, args)
    ...
    Database.fetch([[
        SELECT bc.id AS card_id, bc.card_number, bc.type AS card_type, bc.active, bc.expires_at,
               bc.created_at, bc.unique_id AS owner_uid, ba.balance, ba.status AS account_status,
               ba.iban, c.firstname, c.lastname
        FROM bank_cards bc
        LEFT JOIN bank_accounts ba ON ba.id = bc.account_id
        LEFT JOIN characters c ON c.unique_id COLLATE utf8mb4_unicode_ci = bc.unique_id COLLATE utf8mb4_unicode_ci
        ORDER BY bc.created_at DESC
    ]], {}, function(results)
        if not results or #results == 0 then
            if isConsole then print("^1[CARDLIST]^7 Aucune carte trouvée.")
            else ServerUtils.notifyPlayer(src, "Aucune carte trouvée.", "warning") end
            return
        end
        ...
        for _, row in ipairs(results) do
            ...
            if row.firstname and row.lastname then
                owner = string.format("%s %s [%s]", firstname, lastname, ownerUid)
            else
                owner = string.format("^1[PERSONNAGE SUPPRIMÉ]^7 [%s]", ownerUid)
            end
            local cardStatus = active and "^2[ACTIVE]^7" or "^1[BLOQUÉE]^7"
            print(string.format(" ^3#%d^7 %s ^6%s^7 | Type: %s | Owner: %s | Solde: ^2$%.2f^7 | Compte: %s | Expire: %s",
                row.card_id or 0, cardStatus, row.card_number or "?", row.card_type or "?",
                owner, balance, row.account_status or "?", tostring(row.expires_at or "?")))
        end
        print("^5═══════════════════════════════════════^7")
        local msg = string.format("%d carte(s) affichée(s) dans la console.", #results)
        if isConsole then print("^2[CARDLIST]^7 " .. msg)
        else ServerUtils.notifyPlayer(src, msg, "info") end
    end)
end, false)
...
RegisterCommand("deletecard", function(source, args)
    ...
    Database.execute("DELETE FROM bank_cards WHERE id = ?", { cardId }, function(result)
        ...
        if affected and affected > 0 then
            ...
            if isConsole then print("[CARDLIST] " .. msg)
            else
                ServerUtils.notifyPlayer(src, msg, "success")
                Logger:info(string.format("[CARDLIST] Carte #%d supprimée par %s", cardId, PlayerManager.get(src).name))
            end
        else
...
RegisterCommand("blockcard", function(source, args)
    ...
    Database.execute("UPDATE bank_cards SET active = 0 WHERE unique_id = ?", { uid }, function(result)
        local affected = type(result) == "table" and (result.affectedRows or 0) or (result or 0)
        if affected and affected > 0 then
