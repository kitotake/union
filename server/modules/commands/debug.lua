...
RegisterCommand("dbg:player", function(source, args)
    ...
    if not targetId then print("^1[DEBUG] Usage: dbg:player <id>^7"); return end
    local target = PlayerManager.get(targetId)
    if not target then
        print("^1[DEBUG] Joueur " .. targetId .. " introuvable^7")
        ServerUtils.notifyPlayer(src, "Joueur introuvable.", "error"); return
    end
    ...
    if target.currentCharacter then
        ...
        print("    ped_model : " .. tostring(c.ped_model))
...
RegisterCommand("dbg:server", function(source)
    ...
    ServerUtils.notifyPlayer(src, string.format("Serveur: %d joueurs (%d admins, %d modos).",
        stats.total, stats.admins, stats.moderators), "info")
end, false)
...
RegisterCommand("dbg:db", function(source, args)
    ...
    if not uid then
        ...
        ServerUtils.notifyPlayer(src, "Usage: /dbg:db <unique_id>", "error"); return
    end
    Database.fetchOne(
        ...
        { uid }, function(result)
            if not result then
                ...
                ServerUtils.notifyPlayer(src, "UID introuvable en DB.", "error"); return
            end
            local pos = {}
            ...
            print("  ped_model : " .. tostring(result.ped_model))
...
RegisterCommand("dbg:pos", function(source)
    ...
    if not player or not player.currentCharacter then
        ServerUtils.notifyPlayer(src, "Aucun personnage actif.", "warning"); return
    end
    local c   = player.currentCharacter
    local pos = c.position or {}
    local msg = string.format("Pos: %.2f / %.2f / %.2f | Heading: %.1f",
        pos.x or 0, pos.y or 0, pos.z or 0, c.heading or 0)
