...
RegisterCommand("givemoney", function(source, args)
    ...
    if not targetId or not amount or amount <= 0 then
        ServerUtils.notifyPlayer(src, "Usage: /givemoney <id> <montant>", "error"); return
    end
    local target = PlayerManager.get(targetId)
    if not target or not target.currentCharacter then
        ServerUtils.notifyPlayer(src, "Joueur ou personnage introuvable.", "error"); return
    end
...
RegisterCommand("removemoney", function(source, args)
    ...
    local amount   = tonumber(args[2])
    if not targetId or not amount or amount <= 0 then
        ServerUtils.notifyPlayer(src, "Usage: /removemoney <id> <montant>", "error"); return
    end
    local target = PlayerManager.get(targetId)
    if not target or not target.currentCharacter then
        ServerUtils.notifyPlayer(src, "Joueur ou personnage introuvable.", "error"); return
...
RegisterCommand("checkbalance", function(source, args)
    ...
    local targetId = tonumber(args[1])
    if not targetId then ServerUtils.notifyPlayer(src, "Usage: /checkbalance <id>", "error"); return end
    local target = PlayerManager.get(targetId)
    if not target or not target.currentCharacter then
        ServerUtils.notifyPlayer(src, "Joueur ou personnage introuvable.", "error"); return
    end
    Bank.getBalance(target.currentCharacter.unique_id, function(balance)
        local msg = string.format("[BANK] %s (%s) — Solde : $%s", target.name, target.currentCharacter.unique_id, balance)
        print("^3" .. msg .. "^7")
...
RegisterCommand("setbalance", function(source, args)
    ...
    local amount   = tonumber(args[2])
    if not targetId or not amount or amount < 0 then
        ServerUtils.notifyPlayer(src, "Usage: /setbalance <id> <montant>", "error"); return
    end
    local target = PlayerManager.get(targetId)
    if not target or not target.currentCharacter then
        ServerUtils.notifyPlayer(src, "Joueur ou personnage introuvable.", "error"); return
    end
    Database.execute("UPDATE bank_accounts SET balance = ? WHERE unique_id = ? AND type = 'personal'",
        { amount, target.currentCharacter.unique_id }, function(result)
            if result then
                ServerUtils.notifyPlayer(src, string.format("Solde de %s défini à $%s.", target.name, amount), "success")
                ServerUtils.notifyPlayer(targetId, string.format("Votre solde a été modifié par un admin : $%s.", amount), "info")
            else
...
RegisterCommand("banktop", function(source)
    ...
    Database.fetch([[
        ...
        JOIN characters c ON c.unique_id COLLATE utf8mb4_unicode_ci = ba.unique_id COLLATE utf8mb4_unicode_ci
        WHERE ba.type = 'personal' ORDER BY ba.balance DESC LIMIT 10
    ]], {}, function(results)
        if not results or #results == 0 then
            ServerUtils.notifyPlayer(src, "Aucun compte trouvé.", "warning"); return
        end
        print("^5[BANKTOP] ═══════ Top 10 Richesse ═══════^7")
        for i, row in ipairs(results) do
            print(string.format("^3#%d^7 %s %s ^2$%s^7 [%s]",
                i, row.firstname or "?", row.lastname or "?", row.balance or 0, row.unique_id or "?"))
        end
        print("^5═══════════════════════════════════════^7")
