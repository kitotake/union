...
RegisterNetEvent("characters:selectCharacter", function(charId)
    ...
    if player.isSpawned then
        Logger:warn("[charManager] Joueur déjà spawné, sélection ignorée src=" .. src); return
    end
    local characterId = tonumber(charId)
    if not characterId or characterId <= 0 then
        TriggerClientEvent("characters:error", src, "ID de personnage invalide."); return
    end
    local owned = false
    for _, char in ipairs(player.characters or {}) do
        if char.id == characterId then owned = true; break end
    end
    if not owned then
        Logger:warn("[charManager] Tentative de sélection d'un personnage non possédé — src=" .. src)
        TriggerClientEvent("characters:error", src, "Personnage introuvable."); return
    end
    if not CharacterSelect.isAvailable(characterId) then
        ...
        TriggerClientEvent("characters:error", src, "Ce personnage est déjà utilisé."); return
    end
    Character.select(player, characterId, function(success, character)
        if not success then
            TriggerClientEvent("characters:error", src, "Erreur lors de la sélection du personnage."); return
        end
        Logger:info(("[charManager] Personnage sélectionné : %s %s pour %s"):format(
            character.firstname or "?", character.lastname or "?", player.name))
...
RegisterNetEvent("union:character:requestCreation", function()
    ...
    local player = PlayerManager.get(src)
    if not player then
        Logger:warn("[charManager] requestCreation : joueur introuvable src=" .. src); return
    end
    if GetResourceState("kt_character") ~= "started" then
        ...
        ServerUtils.notifyPlayer(src, "L'interface de création n'est pas disponible.", "error"); return
    end
...
RegisterNetEvent("kt_character:characterCreated", function(data)
    ...
    local player = PlayerManager.get(src)
    if not player then
        Logger:warn("[charManager] kt_character:characterCreated — joueur introuvable src=" .. src); return
    end
    if not data then
        TriggerClientEvent("characters:error", src, "Données de personnage manquantes."); return
    end
    ...
    Character.create(player, data, function(success, characterId, uniqueId)
        if success then
            Logger:info(("[charManager] Personnage créé ID=%s UID=%s"):format(tostring(characterId), tostring(uniqueId)))
...
RegisterNetEvent("characters:playerReady", function()
    Logger:debug("characters:playerReady reçu de src=" .. tostring(source) .. " (ignoré)")
end)
