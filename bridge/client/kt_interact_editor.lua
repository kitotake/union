...
RegisterNetEvent("kt_interact_data:loadAll", function(interactions)
    if type(interactions) ~= "table" then
        print("^1[BRIDGE:kt_interact] loadAll : données invalides^7")
        return
    end
    ...
    print(("^2[BRIDGE:kt_interact] %d interaction(s) chargée(s)^7"):format(count))
end)
...
AddEventHandler("onResourceStop", function(r)
    if r == "kt_interact" then
        Bridge.InteractData._interactions = {}
        Bridge.InteractData._loaded       = false
        print("^3[BRIDGE:kt_interact] kt_interact arrêté — cache vidé^7")
    end
end)